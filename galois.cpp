...
namespace Galois{
        ...
        for(j=0;j<count;j++){
            ...
            for(i=j+1;i<count;i++){
                ...
                for(int k=j+1;k<count;k++){
                    Nomial aa=(*buf->get(i,k)-*buf->get(j,k)**n);
                    Nomial *m=&aa;
                    *buf->set(i,k,m);
                }
            }
    ...
    Polynomial *Polynomial::solve(Polynomial *lu)
    {
        ...
        for(j=lu->cols-1;j>=0;j--){
            ...
            Nomial aa=(*ret->get(j)/(*lu->get(j,j)));
            ret->set(j,&aa);
        }
        return(ret);
    }
    ...
    int BCH::decode(int syndrome_base)
    {
        ...
        if(err){
            ...
            for(j=0,c=0;j<this->rows;j++){
                ...
                for(i=1;i<errors;i++){
					Galois::Nomial a=(*sigma+*err->get(i)*
                            *this->_gf->exp2nomial(j*i));
                    sigma=&a;
                }
                Galois::Nomial aa=(*sigma+*this->_gf->exp2nomial(j*i));
                sigma=&aa;
